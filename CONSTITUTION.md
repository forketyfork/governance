...
| `docs/ARCHITECTURE.md` | How the software is built. Layers, modules, dependencies, data flow, and patterns. Includes cross-Land dependencies consumed by this project. Source of truth for HOW. | `/architecture` command           | `/architecture` command or manually during `/implement` workflow |
| `docs/decisions/`      | Architectural decision records (`DR-NNN-slug.md`) with one decision per file. Enables selective loading of architecture rationale by humans and agents.                | `/architecture` command           | `/architecture` command or during `/implement` workflow          |
